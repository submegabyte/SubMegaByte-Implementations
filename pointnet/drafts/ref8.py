...
class ModelNet10Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if points.shape[0] < self.num_points:
            choice = np.random.choice(points.shape[0], self.num_points, replace=True)  # Sample with replacement
            points = points[choice, :]
        else:
            choice = np.random.choice(points.shape[0], self.num_points, replace=False)  # Sample without replacement
            points = points[choice, :]
